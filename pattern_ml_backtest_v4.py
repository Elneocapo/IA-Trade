...
La red entrena SOLO con el primer tramo histórico. El último tramo queda
intacto hasta el final y sirve como examen real de generalización.
...
def build_sequences(data):
    ...
    for end in range(LOOKBACK_BARS - 1, len(data) - INTRADAY_HORIZON_BARS):
        ...
        ret = future_return[end]
        vol = volatility[end]
        if not np.isfinite(window).all() or not np.isfinite(ret) or not np.isfinite(vol):
            continue
    ...
    for epoch in range(1, TRAIN_EPOCHS + 1):
        ...
        if score > best_score + 0.0001:
            ...
            if patience >= TRAIN_PATIENCE:
                print(f"Parada temprana en época {epoch}: la validación dejó de mejorar.")
                break
...
def main():
    ...
    for col, cls in enumerate(model.classes_):
        probabilities[:, int(cls)] = probabilities_raw[:, col]

    predictions = np.argmax(probabilities, axis=1)
    test_score = balanced_accuracy_score(y_test, predictions) * 100

    close = data.loc[test_indices, "Close"].astype(float)
    final_value, trades, equity = simulate_strategy(close, probabilities)
    buy_hold = INITIAL_CASH * (close.iloc[-1] / close.iloc[0])

    print()
    print("=== EXAMEN FINAL (DATOS NUNCA VISTOS) ===")
    print(f"Validación durante entrenamiento: {best_val:.2%}")
    print(f"Balanced accuracy TEST:            {test_score:.2f}%")
    ...
    if final_value > buy_hold and test_score > 50:
        print("🟢 Hay una señal interesante: ha generalizado en este periodo nunca visto.")
    elif test_score > 50:
        print("🟡 La red detecta algo en el test, pero la estrategia todavía no supera Buy & Hold.")
    else:
        print("🔴 En el examen final no demuestra una ventaja estable.")

    print()
    print("--- ENTRENAMIENTO ---")
    print(f"Épocas realizadas: {len(losses)}")
    print("La gráfica muestra cómo aprendió sin enseñar nunca el test final.")
