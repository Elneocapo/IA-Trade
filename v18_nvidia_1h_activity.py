"""IA-Trade V18 - NVDA only, sequential 1H activity/robustness experiment.

Goal: increase the number of paper trades without selecting a threshold that
only works on one validation slice. Final test remains blind.

This is an experiment, not a live trading system.
"""
...
HOLD_CANDIDATES=(2,3,4,5,6,8,10)
THRESHOLD_CANDIDATES=(0.00005,0.0001,0.00015,0.0002,0.00025,0.00035,0.0005,0.00075,0.001,0.00125,0.0015,0.002)
            ...
            for hold in HOLD_CANDIDATES:
                for tf in (False,True):
                    for vf in (False,True):
                        ...
                        # Require useful activity but keep economics primary.
                        score=r-.32*abs(min(dd,0))+.018*max(sh,0)+min(tr,30)*0.0004
                        if tr<15: score-=.008*(15-tr)
                        if wr<.40 and tr>=10: score-=.012*(.40-wr)
