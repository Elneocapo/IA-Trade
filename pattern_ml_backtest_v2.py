"""IA de patrones con etiquetas estrictas y walk-forward intradía.

Este experimento solo etiqueta movimientos futuros suficientemente grandes:
- subida: rendimiento futuro >= max(0.30%, volatilidad reciente)
- bajada: rendimiento futuro <= -max(0.30%, volatilidad reciente)
- movimientos pequeños: se descartan del entrenamiento y de la evaluación.

Todo el proceso es investigación histórica y simulación, sin broker.
"""
...
def build_sequences(data):
    ...
    if not sequences:
        raise ValueError("No hay suficientes movimientos grandes para entrenar la IA.")
...
def predict_block(model, scaler, X_test):
    X_scaled = scaler.transform(X_test.reshape(len(X_test), -1))
...
def simulate_strategy(close, probabilities):
    ...
    for price, probability in zip(close.to_numpy(), probabilities):
...
def main():
    print("=== IA-Trade | ETIQUETAS ESTRICTAS + PATRONES + WALK-FORWARD ===")
    ...
    print(f"Etiqueta subida: >= {MIN_MOVE_RETURN:.2%} y >= volatilidad")
    print(f"Etiqueta bajada: <= -{MIN_MOVE_RETURN:.2%} y <= -volatilidad")
    print("Movimientos pequeños: descartados")
    print(f"Entrada IA: >= {ENTRY_PROBABILITY:.0%} probabilidad de subida fuerte")
    print(f"Salida IA: <= {EXIT_PROBABILITY:.0%}")
    ...
    X, y, indices, volatilities, raw_returns = build_sequences(data)
    splits = make_walk_forward_splits(len(X))

    print(f"Secuencias con movimiento grande: {len(X)}")
    print(f"Subidas fuertes:                  {int(y.sum())}")
    ...
    for split_id, train_end, test_start, test_end in splits:
        ...
        probabilities = predict_block(model, scaler, X_test)
    ...
    plt.xlabel("Época")
    plt.ylabel("Pérdida")
    plt.title("Entrenamiento walk-forward con etiquetas estrictas")
    plt.legend()
