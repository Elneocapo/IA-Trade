...
signal-percentile entry, using past predictions only.
"""
...
QUANTILE_CANDIDATES=(.60,.65,.70,.75,.80,.85); WINDOW_CANDIDATES=(24,48,72,120)

def load_data():
 print(f"=== V19 | {ASSET} | {INTERVAL} | loading data... ===",flush=True)
 ...
 print(f"loaded {len(d):,} hourly bars",flush=True)
 return d
...
def predictions(df,start,end):
 f,ret1,a,b,c,d=make_supervised(df); X=f.to_numpy(float); A,B,C,D=[q.to_numpy(float) for q in (a,b,c,d)]; valid=[i for i in range(LOOKBACK_BARS-1,len(f)-6) if start<=i<end and np.isfinite(A[i]) and np.isfinite(B[i]) and np.isfinite(C[i]) and np.isfinite(X[i]).all()]; out=[]; models=None; last=-10**9; t0=time.time(); total=len(valid)
 print(f"sequential predictions: {total:,} bars",flush=True)
 for count,i in enumerate(valid,1):
  ...
  if count==1 or count%200==0 or count==total:
   elapsed=time.time()-t0; eta=(total-count)/max(count/elapsed,1e-9); print(f"prediction {count}/{total} | ETA ~{eta/60:.1f} min",flush=True)
...
def enter(row,panel,i,mode,param):
 ...
 q,w=param; hist=panel.signal.iloc[max(0,i-w):i].dropna()
 if len(hist)<max(12,w//2): return False
 return s>float(hist.quantile(q))
...
def bt(panel,start,end,mode,param,weight,hold):
 ...
 for i,d in enumerate(dates):
  row=panel.loc[d]; price=float(row.price); total=cash+shares*price; signal=enter(row,panel,panel.index.get_loc(d),mode,param); target=weight if signal else 0.
  ...
  if tv<cur*.98 and cur>0:
   ...
   if buy>total*.01: shares+=buy/(price*(1+COST)); cash-=buy*(1+COST); entry=i if entry<0 else entry
  curve.append(cash+shares*price)
 if len(curve)<2:return 0,0,0,0,0,INITIAL_CASH
 curve=np.asarray(curve); peak=np.maximum.accumulate(curve); dd=float(np.min(curve/peak-1)); rets=curve[1:]/curve[:-1]-1; sh=float(np.mean(rets)/(np.std(rets)+1e-12)*np.sqrt(252*6.5)) if len(rets)>20 else 0.; wr=np.nan; return float(curve[-1]/INITIAL_CASH-1),len(trades),dd,sh,wr,float(curve[-1])

def bh(df,s,e):
 p=df[(df.index>=s)&(df.index<e)].Close.astype(float); return float(p.iloc[-1]/p.iloc[0]-1)

def main():
 t=time.time(); df=load_data(); n=len(df); cut=int(n*.60); vc=int(n*.80); test_start=vc+2; print("building walk-forward predictions...",flush=True); panel=predictions(df,cut,n-1); val=panel.index[panel.index<df.index[vc]]; test=panel.index[panel.index>=df.index[test_start]]; vs,ve=val[0],val[-1]; ts,te=test[0],test[-1]
 cand=[]; total_candidates=(1*len(THRESHOLD_CANDIDATES)+len(QUANTILE_CANDIDATES)*len(WINDOW_CANDIDATES))*6*len(HOLD_CANDIDATES); done=0; print(f"testing {total_candidates:,} validation candidates...",flush=True)
  ...
  for param in params:
   for weight in (.25,.35,.50,.70,.90,1.):
    for hold in HOLD_CANDIDATES:
     ...
     cand.append((score,mode,param,weight,hold,r,tr,dd,sh)); done+=1
   if done%100==0: print(f"candidate {done}/{total_candidates}",flush=True)
 best=max(cand,key=lambda x:x[0]); _,mode,param,weight,hold,vr,vt,vdd,vsh=best; print("validation selection complete; running blind final test...",flush=True); fr,ft,fdd,fsh,_,final=bt(panel,ts,te,mode,param,weight,hold); bhv=bh(df,ts,te); bfc=INITIAL_CASH*(1+bhv)
